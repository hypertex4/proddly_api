...
class SimpleDBClass
{
  ...
  public function __construct( $db_conn = array('host'=>DB_HOST,'user'=>DB_USER,'pass'=>DB_PASSWORD,'database'=>DB_NAME) ){
    $host = isset($db_conn['host']) ? $db_conn['host'] : DB_HOST;
    $user = isset($db_conn['user']) ? $db_conn['user'] : DB_USER;
    $pass = isset($db_conn['pass']) ? $db_conn['pass'] : DB_PASSWORD;
  ...
  function Select($SQLStatement){
    ...
    */
    ...
    if ($con) {
      ...
      if($row <1) {
        $result = $row;  
      } else if($row == 1) {  //only one row of data
        $result = array($q->fetch_assoc());         
      } else if( $row >1) {  //multiple rows
